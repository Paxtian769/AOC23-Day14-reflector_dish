...
var input_name = "res://Inputs/small-input.txt"
	...
	while not input_file.eof_reached():
		...
		while pos < line.length():
			if line[pos] == 'O':
				...
				moving_rock.position = Vector3((pos+1)*1.05, 1, rows)
				moving_rocks.add_child(moving_rock)
			elif line[pos] == '#':
				var stationary_rock = stationary_rock_scene.instantiate()
				stationary_rock.position = Vector3((pos+1)*1.05, 1, rows);
